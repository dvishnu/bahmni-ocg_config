select   p.gender,TIMESTAMPDIFF(YEAR, p.birthdate, CURDATE()) AS age, pr.provider_id,pr.name clinician, cn.name diagnosis,count(a.visit_id) new_visits, count(a.visit_id)/denom.visit_count*100 new_visit_percent
FROM
        (SELECT 
        visit_id, patient_id
    FROM
