select ou.age,ou.gender,sum(ou.new_visit) new_visits, (sum(ou.new_visit)/(select count(*) from visit where date_started >'#startDate#' and date_started<'#endDate#'))*100 new_visit_percent from
(select 
	TIMESTAMPDIFF(YEAR,p.birthdate,CURDATE()) AS age,
	p.gender gender, if(b.visit_id is null,1,0) new_visit,1 total  from 
